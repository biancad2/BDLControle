...
class Locacoes extends Component {
    ...
    constructor(props) {
     ...
     componentDidMount() {
        ...
        api.get('/locacoes/')
        .then(response => {
          this.setState({ locacoes: response.data, items: response.data });
        })
    ...
    filterList=(event)=>{
        let items = this.state.locacoes;
        items = items.filter((item)=>{
           
            return item.id_empresa.toString().toLowerCase().search(event.target.value.toLowerCase()) !== -1 || item.id_veiculo.toString().toLowerCase().search(event.target.value.toLowerCase()) !== -1 || item.dt_alugada.toString().toLowerCase().search(event.target.value.toLowerCase()) !== -1 
        });
        this.setState({items: items});
        console.log(items);
    }
    tabRow(){
        return this.state.locacoes.map(function(object, i){
        ...
        <nav className="navbar navbar-expand-md navbar-dark bg-menu" id="menuu">
                ...
                <form method="get" action="" className="centro row">
                      ...
                      <div className="formulario busca col-md-5">
                          ...
                          <input type="text" name="pesquisa" id="pesquisar" size="9" minlength="6" maxlength="7" placeholder="Pesquisar..." data-toggle="tooltip" data-trigger="hover" data-placement="bottom" title="Você pode pesquisar pelo ID da empresa ou do veículo" onChange={this.filterList}/>
                          <span className="box_icone_busca">
                              <i className="fa fa-search icone_busca"></i>
                          </span> 
                      </div>
                      <div className="col-md-5">
                          <label for="pesquisa" className="label-pesquisa"> Data	</label> 
                          <input type="date" name="pesquisa" id="pesquisarr" size="9" minlength="6" maxlength="7" placeholder="Endereço" data-toggle="tooltip" data-trigger="hover" data-placement="bottom" title="Você pode pesquisar pela data de locação cadastrada" onChange={this.filterList}/>
                          <span className="box_icone_busca">
                          <i class="far fa-calendar-alt icone_busca"></i>
                  ...
                  <div className="adc-rem">
                      Adicionar locação: 
                     <Link to={'./incluir-locacao'}> <button className="adc"></button></Link>
                  </div>
                ...
                <table className="table table-md" >
                          <thead className="thead-dark">
                              <tr>
                                  ...
                                  <th>Empresa</th>
                                  <th>Veículo</th>
                                  <th className="text-truncate">Data de locação</th>
                                  <th className="text-truncate">Data de devolução</th>
                                  <th className="text-truncate">Valor</th>
                                  <th>Encerrar</th>
                                  <th></th>
                              </tr>
                          </thead>
