...
class Estoques extends Component {
    ...
    filterList=(event)=>{
        ...
        items = items.filter((item)=>{
            ...
            return item.nm_produto.toString().toLowerCase().search(event.target.value.toLowerCase()) !== -1 || item.id_produto.toString().toLowerCase().search(event.target.value.toLowerCase()) !== -1 
        });
