...
class Manutencoes extends Component {
    ...
    constructor(props) {
     ...
     componentDidMount() {
        ...
        api.get('/manutencoes/')
        .then(response => {
          this.setState({ manutencoes: response.data, items: response.data });
        })
    ...
    filterList=(event)=>{
        let items = this.state.manutencoes;
        
        items = items.filter((item)=>{
           
            return item.tipo_manut.toString().toLowerCase().search(event.target.value.toLowerCase()) !== -1 || item.id_manutencao.toString().toLowerCase().search(event.target.value.toLowerCase()) !== -1 || item.id_veiculo.toString().toLowerCase().search(event.target.value.toLowerCase()) !== -1 || item.id_veiculo.toString().toLowerCase().search(event.target.value.toLowerCase()) !== -1 
        });
                  ...
                  <li className="nav-item">
                    ...
                    <li className="nav-item dropdown active">
                        ...
                        <div className="dropdown-menu" aria-labelledby="dropdown04">
                            ...
                            <Link className="dropdown-item" to="/manutencoes">Manutenções <span className="sr-only">(atual)</span></Link>
